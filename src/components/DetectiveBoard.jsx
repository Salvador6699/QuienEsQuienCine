...
export default function DetectiveBoard({
  ...
  return (
    <div className="w-full h-full flex flex-col overflow-hidden animate-fadeIn">
      ...
      {/* ========================================================================= */}
      {/* 📱 MOBILE VIEW: PINNED TOP (HUD + TITLE) & PINNED BOTTOM (PILLS + INPUT) */}
      {/* ========================================================================= */}
      <div className="sm:hidden flex flex-col h-full overflow-hidden">
        
        {/* --- 1. PINNED TOP CONTAINER (ALWAYS VISIBLE, NO SCROLL) --- */}
        <div className="flex-shrink-0 bg-[#121620]/95 backdrop-blur-md border border-gray-800/90 rounded-b-xl p-2.5 mb-1.5 shadow-xl space-y-2">
          {/* Row 1: Points + Detective Turn + Rendirse + Resolver */}
          <div className="flex items-center justify-between gap-1.5">
            {/* Points counter */}
            <div className="flex items-center gap-1 bg-[#181D2A] px-2 py-1 rounded-lg border border-gray-800">
            ...
            {/* Turn indicator */}
            <div className="flex items-center gap-1 text-xs font-semibold text-amber-300 bg-amber-500/10 px-2 py-1 rounded-lg border border-amber-500/30 truncate max-w-[110px]">
              <span>{activeDetective.avatar}</span>
              <span className="truncate">{activeDetective.name}</span>
            </div>

            {/* Actions: Rendirse & Resolver */}
            <div className="flex items-center gap-1.5">
              <button
                type="button"
                onClick={() => setShowSurrenderModal(true)}
                title="Rendirse en esta ronda"
                className="px-2 py-1 rounded-lg bg-rose-500/15 hover:bg-rose-500/25 border border-rose-500/40 text-rose-300 font-bold text-xs flex items-center gap-1 active:scale-95 transition-all cursor-pointer"
              >
                <span>🏳️</span>
                <span>Rendirse</span>
              </button>
              ...
              <button
                ...
                className="px-2.5 py-1 rounded-lg bg-emerald-500 hover:bg-emerald-400 text-gray-950 font-bold text-xs flex items-center gap-1 shadow-md shadow-emerald-500/30 active:scale-95 transition-all cursor-pointer"
              >
          ...
          {/* Row 2: Clues Quick Access (-20 pts each) */}
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className={`py-1 px-2 rounded-lg text-xs font-semibold flex items-center justify-center gap-1.5 border transition-all cursor-pointer ${
          ...
          {/* Row 3: Interrogation Title Header (ALWAYS VISIBLE) */}
          <div className="pt-1 border-t border-gray-800/70 flex items-center justify-between text-xs">
            <div className="flex items-center gap-1.5 text-gray-200 font-bold">
              <HelpCircle className="w-3.5 h-3.5 text-amber-400" />
              <span className="uppercase text-[11px] tracking-wide">Interrogatorio con el Árbitro IA</span>
            </div>
        ...
        {/* --- 2. MIDDLE SCROLLABLE QUESTIONS LOG (ONLY THIS SCROLLS) --- */}
        <div className="flex-1 overflow-y-auto min-h-0 px-1 py-1 space-y-2 scrollbar-thin">
          {questionHistory.length === 0 ? (
            ...
            questionHistory.map((item, index) => (
              <div key={index} className="bg-[#181D2A] border border-gray-800/70 rounded-xl p-2.5 text-xs animate-fadeIn shadow-sm">
                ...
                <div className="text-gray-100 font-medium mb-1.5 pl-0.5">
                  "{item.question}"
                </div>
          ...
          {isAsking && (
            <div className="bg-[#181D2A]/70 border border-gray-800 rounded-xl p-2 flex items-center gap-2 animate-pulse text-xs text-gray-400">
              <Sparkles className="w-4 h-4 text-amber-400 animate-spin" />
              <span>Consultando ficha técnica con Gemini IA...</span>
            </div>
          )}
          <div ref={logEndRef} />
        </div>

        {/* --- 3. PINNED BOTTOM CONTAINER (ALWAYS VISIBLE, NO SCROLL) --- */}
        <div className="flex-shrink-0 bg-[#151926] border-t border-gray-800 rounded-t-xl pt-2 pb-2 px-2 shadow-2xl space-y-2">
          {/* Quick Suggestions Horizontal Scroll */}
          <div className="overflow-x-auto whitespace-nowrap scrollbar-none flex gap-1.5 pb-0.5">
          ...
          >
            <input
              ...
              disabled={isAsking}
              className="flex-1 bg-[#181D2A] border border-gray-800 rounded-xl px-3 py-2 text-xs text-gray-200 placeholder-gray-500 focus:outline-none focus:border-amber-500/60 transition-colors"
            />
            <button
              type="submit"
              disabled={isAsking || !question.trim()}
              className="px-3.5 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-gray-950 font-bold text-xs transition-all cursor-pointer disabled:opacity-50 flex items-center justify-center"
            >
      ...
      {/* ========================================================================= */}
      {/* 💻 DESKTOP VIEW: 3-COLUMN STATS BAR & SPLIT MEDIA + CHAT                  */}
      {/* ========================================================================= */}
      <div className="hidden sm:flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-shrink-0 grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
          ...
          {/* Points, Surrender, and Resolver Buttons */}
          <div className="bg-[#121620]/90 border border-gray-800 rounded-xl p-3 flex items-center justify-between">
            <div>
              <div className="text-[10px] text-gray-500 uppercase font-semibold flex items-center gap-1">
                <span>Pozo</span>
                <span className="text-[10px] text-amber-400/80">(-1 pt/pregunta)</span>
              </div>
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => setShowSurrenderModal(true)}
                className="px-3 py-2 rounded-lg bg-rose-500/15 hover:bg-rose-500/25 border border-rose-500/40 text-rose-300 font-bold text-xs flex items-center gap-1.5 transition-all cursor-pointer"
              >
                <span>🏳️</span>
                <span>Rendirse</span>
              </button>
        ...
        {/* Desktop Main Split: Left Clues Sidebar & Right Interrogation */}
        <div className="flex-1 min-h-0 grid grid-cols-12 gap-4 overflow-hidden">
          {/* Left Sidebar: Real Clues */}
                  ...
                  >
                    <div className="relative w-full aspect-video overflow-hidden bg-black flex items-center justify-center">
                      ...
                      <div className="absolute bottom-0 inset-x-0 h-5 bg-black/90 z-10 flex items-center px-2">
                        <span className="text-[8px] font-mono tracking-wider text-cyan-400/90 uppercase">Encuadre Escénico</span>
                      </div>
                    </div>
          ...
          {/* Right Area: Interrogation Chat Room */}
          <div className="lg:col-span-8 flex flex-col bg-[#121620]/90 border border-gray-800/80 rounded-2xl overflow-hidden backdrop-blur-xl h-full">
            {/* Header */}
            <div className="px-3 py-2.5 border-b border-gray-800 flex items-center justify-between bg-[#151926] flex-shrink-0">
              <div className="flex items-center gap-2">
            ...
            {/* Messages List */}
            <div className="flex-1 p-3 overflow-y-auto space-y-2.5 scrollbar-thin min-h-0">
              {questionHistory.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center p-4 text-gray-500">
                  <HelpCircle className="w-8 h-8 mb-2 opacity-30 text-amber-400" />
                ...
                questionHistory.map((item, index) => (
                  <div key={index} className="bg-[#181D2A] border border-gray-800/70 rounded-xl p-3 text-xs animate-fadeIn">
                    <div className="flex items-center justify-between mb-1">
                      <span className="font-semibold text-gray-300 flex items-center gap-1">
                        <span>{item.detective.avatar}</span>
                        <span>{item.detective.name}</span>
            ...
            <div className="px-3 py-1.5 bg-[#151926]/70 border-t border-gray-800/60 overflow-x-auto whitespace-nowrap scrollbar-none flex gap-1.5 flex-shrink-0">
              {QUICK_QUESTIONS.map((q) => (
                <button
                  key={q}
                  type="button"
                  onClick={() => handleAsk(q)}
                  disabled={isAsking}
                  className="text-[11px] px-2.5 py-0.5 rounded-full bg-[#1e2436] hover:bg-[#283047] text-gray-300 hover:text-amber-300 border border-gray-700/60 transition-colors cursor-pointer flex-shrink-0 disabled:opacity-50"
                >
                  {q}
                </button>
              ))}
            ...
            {/* Input Bar Form */}
            <div className="p-3 border-t border-gray-800 bg-[#151926] flex-shrink-0">
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  handleAsk();
              ...
              >
                ...
                <button
                  type="submit"
                  disabled={isAsking || !question.trim()}
                  className="px-4 py-2 rounded-xl bg-amber-500 hover:bg-amber-400 text-gray-950 font-bold text-xs sm:text-sm transition-all cursor-pointer disabled:opacity-50 flex items-center gap-1.5"
                >
      ...
      {/* ========================================================================= */}
      {/* 🏳️ SURRENDER CONFIRMATION MODAL                                           */}
      {/* ========================================================================= */}
      {showSurrenderModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-sm bg-[#151926] border border-rose-500/40 rounded-2xl p-5 shadow-2xl text-center relative overflow-hidden">
            <div className="w-12 h-12 rounded-full bg-rose-500/20 text-rose-300 flex items-center justify-center mx-auto mb-3 text-2xl">
              🏳️
            </div>
            ...
            <p className="text-xs text-gray-400 mb-5 leading-relaxed">
              Se revelará la película secreta a todos los jugadores y la ronda finalizará sin puntos para los detectives.
            </p>
            <div className="flex gap-2.5">
              <button
                type="button"
                onClick={() => setShowSurrenderModal(false)}
                className="flex-1 py-2.5 px-3 rounded-xl bg-gray-800 hover:bg-gray-700 text-gray-300 text-xs font-semibold transition-colors cursor-pointer"
              >
                Seguir jugando
              </button>
              <button
                ...
                className="flex-1 py-2.5 px-3 rounded-xl bg-gradient-to-r from-rose-600 to-rose-500 hover:from-rose-500 hover:to-rose-400 text-white text-xs font-bold shadow-lg shadow-rose-600/30 transition-all cursor-pointer"
              >
                Sí, rendirse
              </button>
            </div>
            ...
            {activeClueModal === 'soundtrack' && (
              <div>
                ...
                <button
                  type="button"
                  onClick={() => setActiveClueModal(null)}
                  className="w-full py-2.5 rounded-xl bg-gray-800 hover:bg-gray-700 text-gray-200 text-xs font-semibold cursor-pointer"
                >
                  Cerrar
                </button>
              </div>
        ...
        >
          <div className="relative w-full max-w-3xl">
            ...
            <button
              type="button"
              onClick={() => setZoomImage(false)}
              className="absolute -top-3 -right-3 p-2 rounded-full bg-cyan-500 text-gray-950 font-bold shadow-lg cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
